...
### Phase 1 - 핵심 버그
- [x] 게임 플레이 중 화면 상단에 MM:SS 형식 타이머가 실시간 표시됨
- [x] 마우스 클릭(누르고 있기) 시 전진, 마우스 좌우 이동 시 방향 회전
- [x] 마우스를 놓으면 이동이 멈춤, 포인터 락이 발생하지 않음
- [x] 여동생 사운드 3종이 랜덤 간격(3~8초)으로 재생됨
- [x] 멀리서도 작은 볼륨으로 사운드가 들림
- [x] 검사 캐릭터가 항상 달리기 애니메이션을 재생함
- [x] 검사 캐릭터의 뒷모습이 사용자에게 보임

### Phase 2 - UI/UX
- [x] StartScreen이 겨울+귀멸의 칼날 테마로 리디자인됨
- [x] ClearScreen이 테마에 맞게 리디자인됨
- [x] 게임 중 HUD가 반투명 + 블러 스타일로 표시됨

### Phase 3 - 미니맵 + 이펙트
- [x] 우하단에 원형 미니맵이 표시되고 플레이어/여동생/혈귀 위치가 보임
- [x] 플레이어 이동 시 바닥에 발자국이 나타남
- [x] 발자국이 4~5초 후 페이드아웃됨
- [x] 눈이 천천히 자연스럽게 내림 (기존 눈보라 + 느린 눈 레이어)

### Phase 4 - 혈귀
- [x] 혈귀 5마리가 맵에 랜덤 배치되어 걷기 애니메이션으로 배회함
- [x] 혈귀 근접 시(15m 이내) 화면이 어두워지고 붉은 기운이 감돌음
- [x] 혈귀 충돌 시(2m 이내) 게임이 종료됨
- [x] 게임오버 UI가 표시됨

### Phase 5 - 전환 이펙트
- [x] 여동생 발견 시 밝아지는 전환 효과 후 ClearScreen 표시
- [x] 혈귀 충돌 시 붉은 번쩍임 + 어두워지는 전환 효과 후 GameOverScreen 표시
- [x] 전환 효과 중에는 게임 입력이 비활성화됨
